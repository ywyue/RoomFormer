...
from .losses import custom_L1_loss, angle_loss, sigmoid_focal_loss
...
class DeformableFloormer(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_polys, num_feature_levels,
                 aux_loss=True, with_poly_refine=False, two_stage=False):
        ...
        Parameters:
            ...
            num_queries: number of object queries, ie detection slot. This is the maximal number of possible corners
                         in a single image.
            num_polys: number of maximal number of maximal number of possible polygons in a single image. 
            ...
            two_stage: two-stage model (First stage: detect corners as initial object queries, second stage: sort corners)
        """
        super().__init__()
        ...
        # if two-stage, the last class_embed and coords_embed is for proposal generation (initial object queries)
        ...
        if with_poly_refine:
            ...
            # hack implementation for iterative polygon refinement
            ...
            self.transformer.decoder.coords_embed = None
        if two_stage:
            # hack implementation for two-stage
            self.transformer.decoder.class_embed = self.class_embed
            for coord_embed in self.coords_embed:
                nn.init.constant_(coord_embed.layers[-1].bias.data[2:], 0.0)

        # self.transformer.tgt_coords_embed = MLP(hidden_dim, hidden_dim, 2, 3)
        # self.transformer.tgt_class_embed = nn.Linear(hidden_dim, num_classes)

        # the attention mask is used to prevent object queries in one polygon attending to another polygon
        ...
        for i in range(self.num_queries_per_poly):
    ...
    def forward(self, samples: NestedTensor):
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact = self.transformer(srcs, masks, pos, query_embeds, self.attention_mask.to(src.device))

        num_layer = hs.shape[0]
        outputs_class = enc_outputs_class.reshape(num_layer, bs, self.num_polys, self.num_queries_per_poly)
        ...
        # outputs_class = enc_outputs_class[-1]
        # outputs_class = outputs_class.view(bs, self.num_polys, self.num_queries_per_poly)
        # outputs_coord = inter_references[-1].view(bs, self.num_polys, self.num_queries_per_poly, 2)
        ...
        #     tmp = self.coords_embed[lvl](hs[lvl])
        ...
        #     outputs_classes.append(outputs_class.view(bs, self.num_polys, self.num_queries_per_poly))
        #     outputs_coords.append(outputs_coord.view(bs, self.num_polys, self.num_queries_per_poly, 2))
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices):
        ...
        # TODO: hack coding for different loss or loss weights
        # loss_ce = sigmoid_focal_loss(src_logits, target_classes_onehot, num_boxes, alpha=self.focal_alpha, gamma=2) * src_logits.shape[1]
        # loss_ce = F.binary_cross_entropy_with_logits(src_logits, target_classes, pos_weight=self.pos_weight)
        # class_weights = target_classes * 2 + 1
        # loss_ce = F.binary_cross_entropy(src_logits.squeeze(-1).sigmoid(), target_classes, weight=class_weights)
    ...
    def loss_polys(self, outputs, targets, indices):
        ...
        # not helpful
        # loss_angle = angle_loss(src_polys.flatten(1,2), target_polys, target_len)
