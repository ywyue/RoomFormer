...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        # self.tgt_coords_embed = None
        # self.tgt_class_embed = None
        # self.tgt_output_norm = nn.LayerNorm(d_model)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, tgt_masks=None):
        ...
        if self.two_stage:
            ...
            enc_outputs_coord_unact = self.decoder.coords_embed[self.decoder.num_layers](output_memory) + output_proposals
            ...
            topk_class = torch.gather(enc_outputs_class, 1, topk_proposals.unsqueeze(-1))
            ...
            # tgt_output = self.tgt_output_norm(tgt)
            # enc_outputs_class = self.tgt_class_embed(tgt_output)
            # tmp = self.tgt_coords_embed(tgt_output)
            # enc_outputs_coord_unact = tmp
            # enc_outputs_coord_unact = tmp + inverse_sigmoid(reference_points)
        ...
        if self.two_stage:
            ...
            return hs, init_reference_out, inter_references_out, topk_class, topk_coords_unact
        return hs, init_reference_out, inter_references_out, inter_classes, None
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False):
        ...
        self.pos_trans = nn.Linear(256, 256)
        self.pos_trans_norm = nn.LayerNorm(256)


    def get_query_pos_embed(self, proposals):
        # num_pos_feats = 128
        num_pos_feats = 128
        temperature = 10000
        scale = 2 * math.pi
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.coords_embed is not None:
                tmp = self.coords_embed[lid](output)
                classes = self.class_embed[lid](output)
                if reference_points.shape[-1] == 4:
            ...
            if self.return_intermediate:
                ...
                intermediate_classes.append(classes)
