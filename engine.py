    ...
    for batched_inputs in metric_logger.log_every(data_loader, 10, header):
        ...
        for i in range(bs):
            ...
            for j in range(fg_mask_per_scene.shape[0]):
                ...
                # if len(valid_corners_per_room)>0 and pred_room_label_per_scene[j] not in [16,17]:
                    corners = valid_corners_per_room.cpu().numpy()
                    corners = corners * 255
...
@torch.no_grad()
def evaluate_floor(model, dataset_name, data_loader, device, output_dir, plot_pred=True, plot_density=True, plot_gt=True):
    model.eval()
        ...
        for i in range(pred_logits.shape[0]):
            ...
            for j in range(fg_mask_per_scene.shape[0]):
                ...
                if len(valid_corners_per_room)>0:
                    ...
                    if plot_density:
                        pred_room_map = plot_room_map(corners, pred_room_map)
